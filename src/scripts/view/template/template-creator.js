        ...
        return `<h2 class="text-heading">${data.name}</h2>
            <div class="detail">
                <div class="images-detail">
                    <div>
                        <img class="img-res2" alt="${data.name}" src="${
            CONFIG.BASE_IMAGE_URL_M
        }${data.pictureId}" crossorigin="anonymous"/>
                    </div>
                    <ul class="detail-info">
                        <li>
                        <p class="description"><strong>Detail restaurant:</strong> <br>${
                            data.description
                        }</p></li>
                        <li><span><i title="address" class="fa fa-map-marker-alt"></i>&emsp;${
                            data.address
                        }, ${data.city}</span></li>
                        <li><span>${this.getRating(data.rating)}&emsp;${
            data.rating
        }</span></li>
                        <li>${data.categories
                            .map(
    ...
    static getRating(rating) {
        ...
        for (let i = 0; i < parseFloat(rating); i++) {
            if (parseFloat(rating) > i && i === parseInt(rating)) {
                stars +=
                    '<i class="fas fa-star-half" aria-hidden="true" style="color: yellow"></i>';
            } else {
                stars +=
